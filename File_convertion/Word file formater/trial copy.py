...
def extract_general_formatting(input_docx, json_file):
    ...
    general_formatting = {
        "font_size": round(mean(font_sizes)) if font_sizes else None,
        "alignment": Counter(alignments).most_common(1)[0][0] if alignments else None,
        "line_spacing": round(mean(line_spacings)) if line_spacings else None,
        "space_before": round(mean(space_before_values)) if space_before_values else None,
        "space_after": round(mean(space_after_values)) if space_after_values else None,
        "left_indent": round(mean(left_indents)) if left_indents else None,
        "right_indent": round(mean(right_indents)) if right_indents else None,
        "first_line_indent": round(mean(first_line_indents)) if first_line_indents else None
    }
